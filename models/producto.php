    ...
    public function obtenerProductosVisibles() {
        ...
        $sql = "
            SELECT
                ...
                COALESCE(paletas.colores_preview, '') AS colores_preview
            FROM productos p
            LEFT JOIN (
                SELECT
                    pe.id_producto,
            ...
            LEFT JOIN (
                SELECT
                    id_producto,
                    GROUP_CONCAT(CONCAT_WS('::', nombre, COALESCE(codigo_hex, '')) ORDER BY nombre SEPARATOR '||') AS colores_preview
                FROM producto_colores
                GROUP BY id_producto
            ) AS paletas ON paletas.id_producto = p.id
            LEFT JOIN (
